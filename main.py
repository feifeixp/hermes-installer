"""
Neowow Studio — cross-platform entry point.
...
log.info("=== Neowow Studio starting === pid=%s py=%s platform=%s frozen=%s",
...
def _macos_install_agent() -> None:
    ...
    if not uv_exe.exists():
        ...
        if uv_sys:
            ...
            raise RuntimeError(
                "找不到 uv 安装工具。请下载最新版 Neowow Studio（已内置 uv）。"
            )
    try:
...
def _start_webui_server_windows(port: int, host: str) -> subprocess.Popen:
    ...
    if not venv_python.exists():
        raise RuntimeError(
            ...
            "请删除 ~/.hermes/hermes-agent/ 后重启 Neowow Studio 重新安装。"
        )
    ...
    *current_mode* is `"local"` or `"remote"`, used by the menu bar to
    ...
    if wv2_err:
        ...
        _alert(
            ...
            "Neowow Studio 需要 Microsoft Edge WebView2 Runtime 才能显示界面。\n\n"
            "请访问以下地址免费下载安装（约 2MB）：\n"
            "https://go.microsoft.com/fwlink/p/?LinkId=2124703\n\n"
            "安装完成后重新启动 Neowow Studio 即可。\n\n"
            f"错误日志保存在：\n{_LOG_PATH}",
        )
        sys.exit(1)
...
def _run_remote_mode(url: str, label: str = ""):
    ...
    """
    title = f"Neowow Studio · {label}" if label else "Neowow Studio (远程)"
...
def main():
    ...
    # install + local server entirely. The Neowow Studio becomes a
    ...
    if _port_in_use(port):
        ...
        if not _confirm(
            "Neowow Studio",
            f"端口 {port} 已被另一个 Neowow Studio 实例占用（窗口可能被遮挡）。\n"
            f"关闭旧实例并打开新的 Neowow Studio 窗口？",
        ):
    ...
    if sys.platform == "win32":
        ...
        if not _is_agent_installed():
            ...
            _show_console()
            print("\n" + "=" * 56, flush=True)
            print("   Neowow Studio 首次启动 — 正在安装必要组件", flush=True)
        ...
        try:
            ...
            _alert("Neowow Studio", f"无法启动 WebUI：{exc}")
            sys.exit(1)
