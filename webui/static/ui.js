...
// ── Offline detection (upstream) ─────────────────────────────────────
// Patches window.fetch to detect network failures and surface a banner
// instead of silent broken-state. Kept verbatim from upstream.
...
// ── 401 redirect (Neowow Coding Plan extension) ──────────────────────
// Two auth modes the server can be running in:
...
//     {error, loginUrl} pointing at app.neowow.studio's OAuth start.
//     We can't tell from status alone which mode the server is in,
//     so we read the body's `loginUrl` field — when present it wins,
//     otherwise fall back to /login. Body-read is best-effort; a
//     non-JSON / already-consumed body falls through to the default.
//
// 'login?…' (no leading slash) so subpath mounts like /hermes/ don't
// escape to the site root — keeps the upstream subpath-mount feature
// working. Also handles iOS PWA standalone where a server-side
// 302→/login would break out of the PWA shell into Safari.
